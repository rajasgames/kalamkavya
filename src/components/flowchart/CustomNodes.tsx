...
export function GeographyNode({ id, data }: NodeProps<EntityNodeData>) {
  ...
  const sourcePos = data.direction === 'LR' ? Position.Right : Position.Bottom;
  return (
    <div style={{ background: 'var(--surface-glass)', backdropFilter: 'var(--backdrop-blur)' }} className="relative flex items-center justify-center gap-2 border border-subtle text-primary font-bold text-sm px-4 py-3 min-w-[130px] rounded-sm shadow-soft hover:shadow-hover hover:border-clay/30 transition-all group">
